...
class Order {
    ...
    //delete cart if cart exist in database for this user, add a new one and returns Cart object
    ...
    //empty array if no Orders in db
    public static function GetAllOrders($limit = 0) {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $ret[] = new Order($row['id'], $row['user_id'], $row['status'], $row['isCart'], $row['paymentType'], $row['name'], $row['surname'], $row['address']);
            }
        }
        return $ret;
    }
    ...
    //this function return:
    //array with all Orders of User
    //empty array if user don't have orders
    ...
    public static function GetOrder($id) {
        ...
        if ($result->num_rows > 0) {
            ...
            return new Order($row['id'], $row['user_id'], $row['status'], $row['isCart'], $row['paymentType'], $row['name'], $row['surname'], $row['address']);
        }
        return null;
    }
    ...
    //empty array if no orders for this status
    ...
    //this function return:
    //array with all Carts
    //empty array if no carts in db
    ...
    //this function returns:
    //null if isCart is different than 0 or 1
    //true if status is 0 or 1
